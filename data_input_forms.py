...
def create_scene():
    def submit_scene(name,actors,important_actors,):
        if name == '':
                    return
    ...
    actors = st.multiselect('Actors in scene',st.session_state.actors, key = 'selected_actors') # fill from actor objects
    important_actors = st.multiselect('Important actors in scene',actors,key = 'important_actors') # fill from actor objects
...
def create_scene_requirements_matrix(scenes:dict,actors:list,scaling:float):
    scene_requirements = [convert_scene_to_one_hot(scenes[scene][0], actors,scenes[scene][1],scaling ) for scene in scenes]
    return np.array(scene_requirements).T

@st.cache
def calculate_overlap(requirements:np.ndarray,availabilities:np.ndarray,):
    return sum(requirements*availabilities)

@st.cache
def convert_scene_to_one_hot(scene:list, actors:list, need_actors:list, scaling:float=1.5):
    one_hot = []
    for actor in actors:
        if actor in scene:
